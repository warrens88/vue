...
## **3.5 Vue 인스턴스 라이프 사이클**
  ...
  - **인스턴스 라이프 사이클이란 뷰의 인스턴스가 생성되어 소멸되기까지 거치는 과정을 의미**합니다. <br>
    ...
    <img src="../Ch03/lifecycle.png" max-width="1000px;">
...
### **옵션**
#### **data 옵션**
  - vue 인스턴스 내부에 직접 이용되지 않고 Vue 인스턴스와 Data옵션에 주어진 객체 사이에 프록시를 두어 처리
  - 내장옵션들은 모두 $식별자를 앞에 붙이고 있는데 이름 충돌을 피하기 위한것. 
  - data 객체에 있는 모든 속성을 프록시 처리함. (데이터가 변경되면 화면이 렌더링됨.)

#### **el 옵션**
  - Vue 인스턴스에 연결할 HTML DOM요소를 지정
  - Vue 인스턴스를 생성 할 때 HTML요소를 미리 지정하여 권장 
  - 주의점 : 여러개 요소에 지정할 수 없다는 것,여러 개 요소 중 첫번째 요소만 연결.

#### **computed 옵션**
  - 계산적 속성(computed property)
  - 지정한 값은 함수, Vue 인스턴스는 프록시 처리하여 마치 속성처럼 취급
  - 콘솔창에서 vmSum.sum 처럼 속성 접근 방식으로 사용할때 정상 실행되는 것을 확인할 수 있음
  - 읽기 전용이라고 생각하기 쉽지만 set 메서드를 지정하면 쓰기 작업도 가능 (getter/setter 메서드를 떠올리면된다.)
  - vue.js 하나의 데이터를 기반으로 다른 데이터를 변경할 필요가 있을때 사용
  - 계산형속성은 종속된 값에 의해 결과값이 캐싱된다는점 ( 여러번 엑세스해도 콘솔에서 타임스템프를 출력하는 코드는 실행되지않음)

#### **메서드**
- 등록된 메서드는 Vue 인스턴스를 이용해 직접 호출할 수 있음
- 콧수염(Mustache) 표현식에서도 사용
- ES6에서 제공하는 화살표 함수 사용 불가
...
#### **v-cloak 디렉티브**
- 내장 디렉티브로 VueJS에서 렌더딩 되기 이 전에 브라우저에서 감추는 역할<br>
  (콧수염 표현식의 템플릿 문자열이 잠깐 사라졌다가 나타는것을 볼 수 있음.)
...
- **인스턴스 생성 : new Vue()**
  ...
  - **created**
    - (화면에 반응성 주입)
    - **vue 인스턴스가 생성된 후에 데이터에 대한 관찰기능 계산적속성, 메서드,감시자 설정이 완료된 후 호출**
    - el,template 속성 확인
...
- **인스턴스를 화면에 부착**
  ...
  - **beforeUpdate**
    ...
    - 추가적인 상태 변경을 수행할 수 있음, 하지만 추가적으로 다시 렌더링 되지 않음
  - **updated**	
    - (화면 재 랜더링 및 데이터 갱신)
    - 데이터의 변경으로 가상 DOM이 다시 렌더링되고 패치된 후에 호출
    - 이 훅이 호출되었을때는 이미 컴포넌트의 DOM이 업데이트된 상태
    - DOM에 종속성이 있는 연산을 이단계에서 수행할 수 있음.
...
- **인스턴스 내용 갱신**
  ...
  - **destory**	
    ...
    - Vue 인스턴스가 제거되기 전에 호출됩니다. 이 훅 호출될 때는 Vue 인스턴스의 모든 디렉티브의 바인딩이 해체되고 이벤트 연결도 모두 제거
