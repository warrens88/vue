  ...
  - *프록시*
    ...
    **즉, 웹클라이언트의 요청 URL을 해당 서버가 아닌 프록시 서버로 요청**<br>
    ...
    : 예제 03-01을 실행하고 콘솔창에서 나오는것 과 같이 동일하게 작동됨을 볼수 있습니다..<br>
      ...
      내장옵션들은 모두 $식별자를 앞에 분이고 있는데 이름 충돌을 피하기 위한것. <br>
...
- 계산형 속성 : vue.js 하나의 데이터를 기반으로 다른 데이터를 변경할 필요가 있을때 사용하는 것
...
위 과정에서 VueJS가 처리되기 이 전 화면에는 {{ }} 표현식 또는 v-if 등을 사용해 보여주지 않을 내용들이 그대로 노출되게 됩니다. <br>
 Vue단에서 렌더링 되기 이 전 까지의 아주 잠깐이지만 어쩔 수 없는 과정입니다. 이를 피하기 위해서 바로  v-cloak을 사용하게 됩니다.
